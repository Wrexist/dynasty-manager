...
function renderReport(seasons: SeasonMetrics[]): string {
  ...
  // News-emission visibility. A persistent zero in either column means the
  // news gating thresholds (AI_LOAN_NEWS_MIN_OVERALL / AI_FA_NEWS_MIN_OVERALL)
  // are filtering out all the sub-systems' actual activity. The underlying
  // systems run every week — these flags track what the user *sees*, not
  // what the AI *does*.
  const totalLoans = seasons.reduce((a, s) => a + s.loanCount, 0);
  if (totalLoans === 0) {
    flags.push(`- 🟠 **No loan news emitted** across ${SEASONS} seasons. AI loans happen (see \`processAILoans\`) but stay below \`AI_LOAN_NEWS_MIN_OVERALL\` (70). Lower the threshold or switch to an aggregated weekly summary if users report the loan tab feels empty.`);
  }
  const totalFaSignings = seasons.reduce((a, s) => a + s.freeAgentSignCount, 0);
  if (totalFaSignings === 0) {
    flags.push(`- 🟠 **No FA signing news emitted** across ${SEASONS} seasons. Same root cause: \`AI_FA_NEWS_MIN_OVERALL\` threshold filters out sub-60 signings.`);
  }

  // Runtime FA pool never carries CP players. cpPool.cursor advancing is
  // necessary but not sufficient — the FA spawn path uses generateFreeAgentPlayer
  // (procedural) rather than drawing from the CP pool.
  ...
  // cpPool.cursor should advance as CP market refreshes consume templates.
  // If it stays at 0 across all seasons the advanceCursor wiring has regressed.
  const finalCursor = seasons[seasons.length - 1]?.freeAgentPool.cursor ?? 0;
  if (finalCursor === 0 && seasons.some((s) => s.freeAgentPool.usedFcIds > 0)) {
    flags.push(
      '- 🔴 **`cpPool.cursor` stuck at 0**: the advanceCursor wiring in the weekly market refresh has regressed. `getActivePool()` will starve over long saves.',
    );
  }

  // Detect the known lastMarketRefreshWeek stall: cursor moves in S1, then
  // barely in S2, then plateaus. Root cause is that `week` resets to 1 each
  // season while `lastMarketRefreshWeek` keeps its S1 value (e.g. 44), so
  // the `week - lastMarketRefreshWeek >= 4` check stays false through most
  // of subsequent seasons. Known follow-up — not fixed yet.
  const cursorProgression = seasons.map((s) => s.freeAgentPool.cursor);
  const plateauStart = cursorProgression.findIndex((v, i) => i > 0 && v === cursorProgression[i - 1]);
  if (plateauStart > 0 && plateauStart < SEASONS - 1 && cursorProgression[plateauStart] > 0) {
    flags.push(
      `- 🟠 **\`cpPool.cursor\` plateaus after S${plateauStart + 1}** (value ${cursorProgression[plateauStart]}) — `
      + 'the `lastMarketRefreshWeek` check uses per-season week numbers that reset each year, so the `>=4` trigger stops firing after the first season\'s final refresh. The refresh condition in `orchestrationSlice.ts` needs a season-aware reset.',
    );
  }
