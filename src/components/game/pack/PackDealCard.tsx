  ...
  return (
    <article className="relative isolate w-[272px] shrink-0 snap-start overflow-hidden rounded-2xl border border-white/10 bg-card p-3">
      ...
      <div className="flex items-center justify-between gap-2">
        <span className="inline-flex items-center gap-0.5 rounded-full border border-primary/20 bg-primary/10 px-2 py-1 text-micro font-bold uppercase tracking-wide text-primary">
          <Plus aria-hidden className="h-3 w-3" />{deal.bonusCards} bonus card{deal.bonusCards === 1 ? '' : 's'}
        </span>
        <span className="inline-flex items-center gap-1 text-micro tabular-nums text-muted-foreground" aria-label={`Bonus ends in ${formatDealRemaining(deal.remainingMs)}`}>
      ...
      <div className="my-3 flex min-h-36 items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 text-micro text-muted-foreground">Includes {deal.bonusCards} bonus</p>
          <p className="mt-3 inline-flex items-center gap-1 rounded-full border border-white/10 bg-black/20 px-2 py-1 text-micro font-semibold tabular-nums text-foreground">
            <ShieldCheck aria-hidden className="h-3 w-3 shrink-0 text-primary" />{1 + deal.bonusCards} × {tier.guaranteedMinOvr}+ OVR
          </p>
          <p className="mt-1 text-micro text-muted-foreground">Guaranteed ratings</p>
        </div>
      </div>
