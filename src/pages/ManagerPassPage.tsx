...
function RewardCell({ tier, track, status, onCollect }: {
  ...
  // The name wraps to two lines instead of truncating: at 375px a cell has
  // ~60px of text width, and "The Tinkerm…" / "Midnight …" named nothing
  // (playthrough 2026-09, R16). The state glyph moved into the status line so
  // the name gets that width back.
  const body = (
    <>
      ...
      <span className="flex-1 min-w-0 text-left">
        <span data-testid="pass-reward-name" className="block text-xs font-semibold text-foreground leading-tight line-clamp-2 break-words">{item.name}</span>
        <span className={cn('mt-0.5 flex items-center gap-1 text-micro', claimable ? 'text-primary font-semibold' : 'text-muted-foreground')}>
          {status === 'claimed' && <Check className="w-3 h-3 text-emerald-400 shrink-0" aria-hidden />}
          {status === 'locked' && <Lock className="w-3 h-3 text-muted-foreground/70 shrink-0" aria-hidden />}
          {status === 'pro_locked' && <Crown className="w-3 h-3 text-primary/70 shrink-0" aria-hidden />}
          {claimable ? t('managerPass.collect') : t(STATE_KEY[status])}
        </span>
      </span>
    </>
  );
  const base = 'flex-1 min-w-0 min-h-[44px] px-2 py-1.5 rounded-lg flex items-center gap-1.5 border';
