/**
 * The press-conference question bank — pure data, loaded on demand.
 *
 * Split out of `pressConferences.ts` so ~65 kB of prose stays out of the main
 * chunk (the audit's #1 headroom item: `mainChunkHardLimitBytes` had ~34 kB
 * left). `pressConferences.ts` dynamic-imports this module shortly after boot
 * and keeps a neutral fallback question for the moment before it lands.
 *
 * Keep this file import-free except for types: it is its own chunk, and a
 * value import from the main chunk evaluated at module top level would be a
 * load-order hazard.
 */
...
export const QUESTIONS: Record<PressConference['context'], PressQuestionDef[]> = {
  post_win: [
    {
