...
export interface GameState {
  ...
  /** Pre-match opposition game plan chosen in Match Prep. Transient — never
   *  persisted; reset each match alongside matchTeamTalk. */
  matchGamePlan: GamePlanId;
  ...
  recordAdPromptShown: () => void;
  recordAdWatched: () => void;
  recordAdPromptDismissed: () => void;
  ...
  startCaptureScenario: (scenario: CaptureScenario) => boolean;
  ...
  /** Hall of Legends — retired greats this save has archived (career source
   *  only; the authored seed set lives in `src/data/legends.ts` and is never
   *  persisted). Appended by `seasonEnd` when a legend-worthy player retires,
   *  capped at `LEGEND_ARCHIVE_CAP`, drawn from by Legend-chance packs. */
