...
- The first full CI run found nine autosave assertions contaminated by retained Storage spy call history, plus an unused former Champions pack cover. Commit `86b62457` restores spies after each test and removes the unused asset. Both affected suites passed locally (104 tests).
- [Final PR checks](https://github.com/Wrexist/dynasty-manager/actions/runs/35535066443) passed: 245 test files, 3,365 tests, with the existing 4 skipped files / 5 skipped tests. Lint, typecheck, docs, localization, pack supply, production build and bundle limits also passed. No failing checks remain on PR #623.
- [TestFlight build 190](https://github.com/Wrexist/dynasty-manager/actions/runs/35535063714) passed its full release gate, archived with Xcode 26.6, and successfully uploaded version 1.6.0 to App Store Connect. Apple processing and device testing are separate from upload success.
- PR #623 was merged into `main` as `c4ee6e00`; local `main` was fast-forwarded. GitHub Pages deployment passed and the live privacy URL returned HTTP 200 with the revised 1.6.0 policy.
- RevenueCat's Dynasty iOS bundle ID and both Apple credential checks were verified. The four consumable products are mapped in the packs offering; the pro entitlement includes monthly, yearly, lifetime, bundle and grandfathered Pro products. No server notifications were shown as received; actual sandbox transactions remain unverified.
- App Review contact details and reviewer navigation notes are saved in the 1.6.0 draft. The Gold IAP review note now correctly distinguishes paid Gold from free daily Bronze/Silver claims.
- Real iPhone/iPad testing, save-upgrade testing, the StoreKit sandbox matrix, final build selection and inclusion of the prepared IAP localization changes remain submission gates. No App Review submission or public app release has been made.
