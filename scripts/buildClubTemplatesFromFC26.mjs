...
function parseCSV(content) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const obj = {};
    for (let j = 0; j < headers.length; j++) obj[headers[j]] = f[j] || '';
    out.push(obj);
  }
  return out;
}
...
for (const [clubId, data] of Object.entries(byClub)) {
  ...
  // Sort players: by overall desc, but keep at least one of each common
  // position near the top so squad coverage stays sensible.
