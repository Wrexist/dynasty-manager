 ...
 *
 * Parses a player CSV (--csv, default FC26_20250921.csv) and emits
...
// Source CSV and output path are overridable so the same generator can build
// the pool from a newer season's export. The FC27 pipeline emits a CSV in this
// exact column shape (scripts/fc27/export_for_game.mjs), which is why the pool
// does not need a second implementation.
//   node scripts/buildNationalPool.mjs --csv data/fc27/FC27_community_pack_input.csv
const argOf = (name, fallback) => {
  const i = process.argv.indexOf(name);
  return i > -1 && process.argv[i + 1] ? process.argv[i + 1] : fallback;
};
const CSV_PATH = join(ROOT, argOf('--csv', 'FC26_20250921.csv'));
const OUT_PATH = join(ROOT, argOf('--out', 'src/data/nationalPlayerPool.ts'));
...
output += `/**\n`;
output += ` * National player pool — top ${MAX_PER_NATION} real players per nationality.\n`;
output += ` * Auto-generated by scripts/buildNationalPool.mjs from ${argOf('--csv', 'FC26_20250921.csv')}.\n`;
