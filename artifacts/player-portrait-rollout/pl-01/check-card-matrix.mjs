...
try{
 ...
 for(const params of [...frames.map(frame=>({frame})),...['xs','sm','md','lg','xl'].map(size=>({size}))]){
  await page.goto('http://127.0.0.1:5180/artifacts/player-portrait-rollout/pl-01/cards.html?'+new URLSearchParams(params));
  await page.waitForSelector('.portrait-preview-grid img[src*="pl-01"]');
  await page.locator('img').evaluateAll(imgs=>{for(const i of imgs)i.loading='eager';return Promise.all(imgs.map(i=>i.decode()));});
  const count=await page.locator('.portrait-preview-grid img[src*="pl-01"]').count();
 ...
 fs.writeFileSync('artifacts/player-portrait-rollout/pl-01/card-matrix-validation.json',JSON.stringify({results,errors},null,2));
