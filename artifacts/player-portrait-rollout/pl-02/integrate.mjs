...
const {players}=JSON.parse(fs.readFileSync('artifacts/player-portrait-rollout/pl-02/manifest.json','utf8'));
...
for(const p of players){
 ...
 const src='/player-portraits/pl-02/'+p.fcId+'.webp';
...
fs.writeFileSync('artifacts/player-portrait-rollout/pl-02/card-players.json',JSON.stringify(fixtures,null,2));
const from='artifacts/player-portrait-sample-25/';const to='artifacts/player-portrait-rollout/pl-02/';
fs.copyFileSync(from+'cards.html',to+'cards.html');
fs.writeFileSync(to+'cards.tsx',fs.readFileSync(from+'cards.tsx','utf8').replace('Your players, in their cards.','Premier League · Batch 02').replace('25 approved portraits','25 new transparent portraits'));
fs.writeFileSync(to+'check-cards.mjs',fs.readFileSync(from+'check-cards.mjs','utf8').replaceAll('artifacts/player-portrait-sample-25/','artifacts/player-portrait-rollout/pl-02/'));
fs.writeFileSync(to+'check-card-matrix.mjs',fs.readFileSync(from+'check-card-matrix.mjs','utf8').replaceAll('artifacts/player-portrait-sample-25/','artifacts/player-portrait-rollout/pl-02/').replaceAll('cutout-v1','pl-02'));
