...
createRoot(document.getElementById('root')!).render(<main style={{padding:32,maxWidth:1400,margin:'auto'}}>
  ...
  <h1 style={{fontSize:40,fontWeight:800,margin:'12px 0'}}>European leagues · 97 new portraits</h1>
  <p style={{color:'#adbdb6',marginBottom:24}}>97 transparent portraits · Actual PlayerCard component · Ratings from the checked-in game roster · Tap a card to cycle its views.</p>
  <nav aria-label="Preview pages" style={{display:"flex",gap:16,marginBottom:24,flexWrap:"wrap"}}><a href="?">All players</a>{Array.from({length:Math.ceil(allPlayers.length/25)},(_,i)=><a key={i} href={"?page="+(i+1)}>Page {i+1}</a>)}</nav><section className="portrait-preview-grid">{players.map(p=><div key={p.id} style={{display:'flex',flexDirection:'column',alignItems:'center'}}>
