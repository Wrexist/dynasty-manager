...
- **Vitest 3.2.4 + jsdom + Testing Library** — 250 test files in `src/test/`
...
├── test/                → 250 test files incl. longevity/stress suites, adversarial
...
2. **`src/store/storeTypes.ts`** — complete `GameState` interface (708 LOC).
...
npm run test         # Vitest (250 test files)
