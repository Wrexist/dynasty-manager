# pl — Polish (App Store)

Storefronts reached: Poland. Apple serves pl metadata to Polish-preferring users in the Polish storefront. The head phrase Polish fans search for the genre is "menedżer piłkarski" (the exact descriptor Top Eleven localizes into its own Polish title), so the title lock-up and Subtitle are built around it. The seasonal tournament token this locale used to carry was retired on 2026-07-19 with the campaign window; tournament names are registered marks and stay out of all consumer-visible copy.

## App Name [28/30]
`Dynasty Manager: Piłka nożna`

## Subtitle [28/30]
`Taktyka, Transfery i Kariera`

## Promotional Text [169/170]
`Nowy sezon, nowa dynastia. Poprowadź jeden z 756 prawdziwych klubów od okresu przygotowawczego po tytuł: każdy transfer, każda taktyka, każda minuta. Bez pasków energii.`

## Keywords [90/100]
`futbol,reprezentacja,karne,liga,trener,sklad,puchar,klub,mlodziez,akademia,symulator,awans`

## Description [1714/4000]
Sezon 2026-27 właśnie ruszył. Wybierz klub, ustaw skład i poprowadź go od okresu przygotowawczego aż po mistrzostwo – każda decyzja należy do ciebie.

Dynasty Manager to głęboki symulator zarządzania piłką nożną, w który zagrasz, gdziekolwiek jesteś. Bez pay-to-win, bez pasków energii, bez pakietów odpoczynku, bez czekania między sesjami. Graj tyle, ile chcesz – pełna głębia menedżerska bez żadnego abonamentu.

Przejmujesz klub, ustawiasz skład, dopracowujesz taktykę i formację, ściągasz zawodników z rynku transferowego i prowadzisz zespół minuta po minucie przez każdy mecz.

Co na ciebie czeka:
- Pełny sezon: okres przygotowawczy, okno transferowe, końcówka rozgrywek i mistrzostwo
- 45 lig i 756 prawdziwych klubów w 37 krajach
- Tryb kariery: aplikuj o posady, negocjuj kontrakty, awansuj – albo wyleć z pracy
- Emocjonujące konkursy rzutów karnych sterowane strzał po strzale
- Mecze na żywo: skład, taktyka, zmiany i przemowy w czasie rzeczywistym
- Transfery i wypożyczenia, negocjacje kontraktów i własna akademia młodzieży
- Rozgrywki kontynentalne, puchary i posada selekcjonera: ponad 50 reprezentacji na turniejach międzynarodowych
- Paczki z zawodnikami w stylu FUT z prawdziwymi piłkarzami

Bez pasków energii. Bez czekania. Tylko piłka, kiedy tylko chcesz.

Dynasty Pro odblokowuje zaawansowane statystyki, własne taktyki, natychmiastową symulację i więcej – z 7 dniami za darmo na start. Pełna ścieżka kariery pozostaje w pełni grywalna także bez abonamentu.

Pobierz za darmo. Zostań trenerem i sięgnij po mistrzostwo.

Polityka prywatności: https://wrexist.github.io/dynasty-manager/privacy.html
Warunki użytkowania (EULA): https://www.apple.com/legal/internet-services/itunes/dev/stdeula/

## What's New (next release note blurb)
Nowy sezon, nowa dynastia. Sezon 2026-27 już wystartował – wybierz klub i poprowadź go od okresu przygotowawczego po tytuł, minuta po minucie. AI na ławce jest sprytniejsze, a kilka błędów w dniu meczu zostało naprawionych. Wciąż bez pasków energii i bez czekania – tylko piłka.

## Screenshot Captions (5)
1. Ponad 50 reprezentacji i turnieje międzynarodowe
2. Od przygotowań po mistrzostwo
3. Karne, od których zamiera serce
4. Skład, taktyka, formacja
5. Prowadź każdą minutę meczu

## Keyword rationale
- **Biggest change: the Subtitle is now `Taktyka, Transfery i Kariera`** — three evergreen category head terms that read as a benefit in the Today-tab ad format (name + icon + subtitle only). The seasonal tournament token that used to occupy this line is gone: that window closed on 19 July 2026, and tournament names — colloquial and formal Polish alike — are registered marks we keep out of every consumer-visible field (APP_STORE_LISTING.md risk #4).
- **Open question for the next pl pass:** "menedżer piłkarski" is the exact descriptor the category incumbent localizes into its own Polish title, and a Subtitle placement outranks a keyword-field token. It currently sits in NEITHER field. Contesting it head-on would mean spending most of the 30-character Subtitle on it — worth testing against the three head terms now there.
- Because the manager head words are covered by the App Name lock-up and the Subtitle, Keywords drop them and re-spend the freed budget on complementary terms; no tournament token is bought back.
- "reprezentacja" now carries national-team intent on its own — the generic word ranks year-round and costs nothing in IP exposure; "trener" moved into Keywords now that the old "trener i taktyka" subtitle is gone.
- "karne", "reprezentacja", "sklad", "transfery", "kariera" are the exact Polish words for penalties, national team, lineup, transfers and career — culturally correct, not literal translations of the English.
- Unaccented forms throughout ("sklad" not "skład") match how Poles type queries without diacritics; the visible copy keeps full Polish spelling.
- "futbol" is kept alongside the App Name's "piłka nożna" because a meaningful slice of younger fans search the borrowed word; Apple treats it as a distinct token.
- Dropped the weakest terms ("gol", "formacja") to make room. Excluded: plurals of included singulars, any club/league proper nouns, and words already in the App Name/Subtitle.
